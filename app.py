...
from symptomsList_model import symptoms_name, symptoms_array, model

app = Flask(__name__)


# main function to render html templates
@app.route('/', methods=['POST', 'GET'])
def main():
    if request.method == 'POST':
        if request.form.get('select_symptom_1').lower() == 'none'\
                and request.form.get('select_symptom_2').lower() == 'none'\
